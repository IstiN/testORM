...
*/
public class VideosDao extends AbstractDao<Videos, Long> {
    ...
    */
    ...
    private Query<Videos> content_VideosQuery;
    ...
    /** @inheritdoc */
    @Override
    protected Long updateKeyAfterInsert(Videos entity, long rowId) {
        entity.setId(rowId);
        return rowId;
    }
    ...
    /** @inheritdoc */
    @Override
    public Long getKey(Videos entity) {
        if(entity != null) {
            return entity.getId();
        } else {
            return null;
    ...
    /** Internal query to resolve the "videos" to-many relationship of Content. */
    public List<Videos> _queryContent_Videos(Long contentId) {
        synchronized (this) {
            if (content_VideosQuery == null) {
                QueryBuilder<Videos> queryBuilder = queryBuilder();
                queryBuilder.where(Properties.ContentId.eq(null));
                content_VideosQuery = queryBuilder.build();
            }
        }
        Query<Videos> query = content_VideosQuery.forCurrentThread();
    ...
    protected Videos loadCurrentDeep(Cursor cursor, boolean lock) {
        Videos entity = loadCurrent(cursor, 0, lock);
    ...
    public Videos loadDeep(Long key) {
        assertSinglePk();
        if (key == null) {
            return null;
        }
    ...
    public List<Videos> loadAllDeepFromCursor(Cursor cursor) {
        int count = cursor.getCount();
        List<Videos> list = new ArrayList<Videos>(count);
    ...
    protected List<Videos> loadDeepAllAndCloseCursor(Cursor cursor) {
        try {
    ...
    public List<Videos> queryDeep(String where, String... selectionArg) {
